...
def get_poll_votes(poll_id: str) -> dict:
    votes = {}
    with get_connection() as conn:
...
def get_channel_polls(channel: str) -> List[Poll]:
    polls = []
    with get_connection() as conn:
...
def vote(poll_id: str, option_index: int, username: str) -> None:
    with get_connection() as conn:
